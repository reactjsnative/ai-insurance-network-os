              ...
              {fixed && (
                <td className="py-2 px-3">
                  <input
                    ...
                    className="bg-slate-800 border border-slate-700 rounded px-2 py-1 text-xs text-emerald-400 font-bold w-24"
                  />
                </td>
              )}
            </tr>
          ...
          {([
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                activeTab === key ? 'bg-blue-600 text-white shadow' : 'text-slate-400 hover:text-white'
              }`}
            >
              {label}
            </button>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            <div className="border-b border-slate-800 pb-3">
              ...
              <p className="text-xs text-slate-400">ค่าจัดงานหน่วย, ค่าแยกหน่วย และค่าพาหนะ</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">ผลงานขั้นต่ำเพื่อดำรงตำแหน่ง (บาท/เดือน)</label>
              ...
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">ค่าพาหนะผู้บริหารหน่วย (บาท/เดือน)</label>
                <input type="number" value={editForm.unitManager.vehicleAllowance}
                  onChange={(e) => setUnit({ vehicleAllowance: num(e.target.value) })}
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl px-3 py-2 text-xs text-white focus:outline-none" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">ค่าแยกหน่วยต่อหน่วย (บาท/เดือน)</label>
            ...
            <div className="mt-4 pt-2">
              <span className="text-xs font-semibold text-slate-300 block mb-2">ขั้นบันไดค่าจัดงานหน่วย:</span>
              <TierEditor
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            <div className="border-b border-slate-800 pb-3">
              ...
              <p className="text-xs text-slate-400">ค่าจัดงานศูนย์ 3 ประเภท, ค่าแยกศูนย์ และโบนัสประจำปี</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">ผลงานขั้นต่ำ (บาท/เดือน)</label>
              ...
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">จำนวนหน่วยแยกขั้นต่ำ</label>
              ...
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">อัตราค่าจัดงานประเภท 2 (%)</label>
              ...
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">เงินเพิ่มค่าแยกศูนย์เดือนแรก</label>
            ...
            <div className="space-y-5">
              <div>
                <span className="text-xs font-semibold text-slate-300 block mb-2">ค่าจัดงานศูนย์ประเภท 1 (%):</span>
                <TierEditor tiers={editForm.centerManager.centerType1Tiers} onChange={(next) => setCenter({ centerType1Tiers: next })} rate />
              </div>
              <div>
                <span className="text-xs font-semibold text-slate-300 block mb-2">ค่าจัดงานศูนย์ประเภท 3 (จำนวนคงที่):</span>
                <TierEditor tiers={editForm.centerManager.centerType3Tiers} onChange={(next) => setCenter({ centerType3Tiers: next })} fixed />
              </div>
              <div>
                <span className="text-xs font-semibold text-slate-300 block mb-2">ค่าแยกศูนย์ (จำนวนคงที่/ศูนย์):</span>
                <TierEditor tiers={editForm.centerManager.centerSeparationTiers} onChange={(next) => setCenter({ centerSeparationTiers: next })} fixed />
              </div>
              <div>
                <span className="text-xs font-semibold text-slate-300 block mb-2">โบนัสประจำปีศูนย์ (%):</span>
          ...
          <div className="bg-slate-900/90 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            <div className="border-b border-slate-800 pb-3">
              ...
              <p className="text-xs text-slate-400">ค่าจัดงานภาค, โบนัสภาค, ค่าแยกภาค และค่าบริหารเป้าหมาย</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">ผลงานขั้นต่ำ (บาท/เดือน)</label>
              ...
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">จำนวนศูนย์แยกขั้นต่ำ</label>
              ...
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">ค่าแยกภาคครั้งเดียว (บาท)</label>
              ...
              <div>
                <label className="text-xs font-semibold text-slate-300 block mb-1">ค่าแยกภาครายเดือน (บาท/เดือน)</label>
            ...
            <div className="space-y-5">
              <div>
                <span className="text-xs font-semibold text-slate-300 block mb-2">ค่าจัดงานภาคประเภท 1 (% ของ FYC ทีม):</span>
                <TierEditor tiers={editForm.groupManager.groupType1Tiers} onChange={(next) => setGroup({ groupType1Tiers: next })} rate />
              </div>
              <div>
                <span className="text-xs font-semibold text-slate-300 block mb-2">ค่าจัดงานภาคประเภท 2 (จำนวนคงที่):</span>
                <TierEditor tiers={editForm.groupManager.groupType2Tiers} onChange={(next) => setGroup({ groupType2Tiers: next })} fixed />
              </div>
              <div>
                <span className="text-xs font-semibold text-slate-300 block mb-2">โบนัสประจำปีภาค (%):</span>
              ...
              <tbody className="divide-y divide-slate-800">
                {auditLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-slate-800/40">
                    ...
                    <td className="py-2.5 px-3 text-slate-300 font-medium">{log.userEmail}</td>
