...
export const Header: React.FC = () => {
  ...
  return (
    <>
      <header id="app_header" className="h-16 border-b border-slate-800 bg-slate-900/90 backdrop-blur-md px-4 sm:px-6 flex items-center justify-between sticky top-0 z-40">
        {/* Left: Brand / Title */}
        ...
        <div className="relative max-w-md w-full mx-4 hidden md:block">
          <div className="relative flex items-center">
            ...
            <input
              ...
              className="w-full bg-slate-950/70 border border-slate-800 focus:border-amber-500/50 rounded-xl pl-9 pr-4 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-amber-500/50 transition-all"
            />
          </div>

          {/* Search Results Dropdown */}
          {filteredSearchMembers.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl overflow-hidden z-50">
              <div className="p-1.5 text-[10px] font-semibold text-slate-400 uppercase tracking-wider px-3 border-b border-slate-800">
        ...
        <div className="flex items-center gap-2 sm:gap-2.5">
          ...
          <div 
            ...
            className="hidden xl:flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-950/80 border border-slate-800 text-[11px] font-medium"
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-80 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-3 z-50">
                ...
                <div className="space-y-2">
                  {notifications.map((n) => (
                    <div key={n.id} className="p-2 rounded-xl bg-slate-950/60 border border-slate-800/80 text-left">
          ...
          {!authUser.isLoggedIn ? (
            ...
            <div className="relative">
              ...
              {/* User Dropdown Menu */}
              {showUserMenu && (
                <div className="absolute right-0 mt-2 w-80 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-2 z-50">
                  {/* Top Profile Summary */}
                  <div className="p-3 bg-slate-950/70 rounded-xl border border-slate-800 mb-2">
                    <div className="flex items-center gap-3">
                      <img
                        src={authUser.avatarUrl || activeUser.avatarUrl}
                        alt={authUser.name}
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-bold text-slate-100 truncate">{authUser.name}</p>
