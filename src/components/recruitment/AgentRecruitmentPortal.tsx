...
export const AgentRecruitmentPortal: React.FC = () => {
  ...
  const getStatusBadge = (status: ApplicationStatus) => {
    switch (status) {
      ...
      case 'document_verified':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-cyan-500/10 text-cyan-700 border border-cyan-500/30">
      ...
      case 'exam_passed':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-indigo-500/10 text-indigo-700 border border-indigo-500/30">
      ...
      case 'approved':
        return (
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-700 border border-emerald-500/30">
  ...
  return (
    <div id="recruitment_portal_container" className="space-y-6 max-w-7xl mx-auto pb-12">
      {/* Header Banner */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-[#fcfdff] via-[#f8fbff] to-[#e0f2fe]/30 border border-sky-50/30 p-6 sm:p-8 shadow-[0_4px_12px_rgba(148,163,184,0.10)]">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-2 bg-[#fcfdff] p-1.5 rounded-xl border border-sky-50/30/80 shadow-[0_1px_3px_rgba(148,163,184,0.08)] self-start md:self-auto">
            <button
              id="tab_recruitment_form"
              onClick={() => { setActiveSubTab('form'); setSelectedApplication(null); }}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${
                activeSubTab === 'form'
                  ? 'bg-blue-600 text-white shadow-[0_1px_3px_rgba(148,163,184,0.08)] shadow-indigo-600/30'
                  : 'text-slate-800 hover:text-blue-700 hover:bg-[#fcfdff]'
              }`}
            >
        ...
        {/* 4 Mini KPI Cards */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-6 border-t border-sky-50/30">
          <div className="bg-[#fcfdff] p-3 rounded-xl border border-sky-50/40 shadow-[0_1px_3px_rgba(148,163,184,0.08)]">
      ...
      {actionAlert && (
        <div className={`p-4 rounded-xl border flex items-center justify-between ${
          ...
          <div className="flex items-center gap-3">
            {actionAlert.type === 'success' ? <CheckCircle2 className="w-5 h-5 text-emerald-700" /> : <BadgeAlert className="w-5 h-5 text-rose-400" />}
      ...
      {activeSubTab === 'form' && (
        <div className="bg-[#fcfdff] border border-sky-50/30/80/80 rounded-2xl p-6 sm:p-8 shadow-[0_4px_12px_rgba(148,163,184,0.10)]">
          {/* Step Progress Tracker */}
          <div className="mb-8">
            <div className="flex items-center justify-between relative">
              <div className="absolute left-0 top-1/2 -translate-y-1/2 h-0.5 bg-[#f0f9ff] w-full -z-0" />
              <div 
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={`flex flex-col items-center gap-2 relative z-10 bg-[#fcfdff] px-3 py-1 rounded-xl transition-all ${
                      isCurrent ? 'scale-105' : ''
                    }`}
                  >
                    <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all ${
                      isPassed 
                        ...
                        : 'bg-[#f0f9ff] border-sky-50/30 text-slate-800'
                    }`}>
                      {isPassed ? <Check className="w-5 h-5 font-bold" /> : <Icon className="w-5 h-5" />}
                    </div>
                    <span className={`text-xs font-semibold whitespace-nowrap ${
                      isCurrent ? 'text-indigo-700' : isPassed ? 'text-emerald-700' : 'text-slate-800'
                    }`}>
                      {s.title}
                    </span>
                  </button>
          ...
          <form onSubmit={handleFormSubmit} className="space-y-6">
            ...
            {currentStep === 1 && (
              <div className="space-y-6 animate-fadeIn">
                <div className="border-b border-sky-50/30 pb-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'คำนำหน้าชื่อ' : 'Prefix'}</label>
                    <select
                      value={formData.prefix}
                      onChange={(e) => setFormData({ ...formData, prefix: e.target.value as any })}
                      className="w-full bg-[#fcfdff] border border-sky-50/30/80 rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:outline-none focus:border-indigo-500"
                    >
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'ชื่อจริง *' : 'First Name *'}</label>
                    <input
                      type="text"
                      placeholder="เช่น ธนภัทร"
                      value={formData.firstName}
                      onChange={(e) => setFormData({ ...formData, firstName: e.target.value })}
                      className={`w-full bg-[#fcfdff] border rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:outline-none ${
                        validationErrors.firstName ? 'border-rose-500' : 'border-sky-50/30 focus:border-indigo-500'
                      }`}
                    />
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'นามสกุล *' : 'Last Name *'}</label>
                    <input
                      type="text"
                      ...
                      className={`w-full bg-[#fcfdff] border rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:outline-none ${
                        validationErrors.lastName ? 'border-rose-500' : 'border-sky-50/30 focus:border-indigo-500'
                      }`}
                    />
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'ชื่อเล่น' : 'Nickname'}</label>
                    <input
                      type="text"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'เลขบัตรประจำตัวประชาชน (13 หลัก) *' : 'National ID Number *'}</label>
                    <input
                      type="text"
                      maxLength={17}
                      ...
                      className={`w-full bg-[#fcfdff] border rounded-lg px-3 py-2.5 text-sm text-slate-800 font-mono focus:outline-none ${
                        validationErrors.nationalId ? 'border-rose-500' : 'border-sky-50/30 focus:border-indigo-500'
                      }`}
                    />
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'วัน/เดือน/ปี เกิด' : 'Birth Date'}</label>
                    <input
                      type="date"
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'เพศ' : 'Gender'}</label>
                    <select
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'เบอร์โทรศัพท์ติดต่อ *' : 'Phone Number *'}</label>
                    <div className="relative">
                      <Phone className="w-4 h-4 text-slate-800 absolute left-3 top-3" />
                      <input
                        type="tel"
                        placeholder="081-234-5678"
                        value={formData.phone}
                        onChange={(e) => setFormData({ ...formData, phone: e.target.value })}
                        className={`w-full bg-[#fcfdff] border rounded-lg pl-9 pr-3 py-2.5 text-sm text-slate-800 focus:outline-none ${
                          validationErrors.phone ? 'border-rose-500' : 'border-sky-50/30 focus:border-indigo-500'
                        }`}
                      />
                    </div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'อีเมล (Email) *' : 'Email *'}</label>
                    <div className="relative">
                      <Mail className="w-4 h-4 text-slate-800 absolute left-3 top-3" />
                      <input
                        type="email"
                        ...
                        className={`w-full bg-[#fcfdff] border rounded-lg pl-9 pr-3 py-2.5 text-sm text-slate-800 focus:outline-none ${
                          validationErrors.email ? 'border-rose-500' : 'border-sky-50/30 focus:border-indigo-500'
                        }`}
                      />
                    </div>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'LINE ID' : 'LINE ID'}</label>
                    <input
                      type="text"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'ที่อยู่ปัจจุบัน' : 'Current Address'}</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'จังหวัด *' : 'Province *'}</label>
                    <input
                      type="text"
            ...
            {currentStep === 2 && (
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'ระดับการศึกษาสูงสุด' : 'Highest Education'}</label>
                    <select
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'สาขาวิชา' : 'Major'}</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'สถาบันการศึกษา *' : 'University / Institute *'}</label>
                    <input
                      type="text"
                      ...
                      className={`w-full bg-[#fcfdff] border rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:outline-none ${
                        validationErrors.university ? 'border-rose-500' : 'border-sky-50/30 focus:border-indigo-500'
                      }`}
                    />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'อาชีพ/ตำแหน่งงานปัจจุบัน *' : 'Current Occupation *'}</label>
                    <input
                      type="text"
                      ...
                      className={`w-full bg-[#fcfdff] border rounded-lg px-3 py-2.5 text-sm text-slate-800 focus:outline-none ${
                        validationErrors.currentOccupation ? 'border-rose-500' : 'border-sky-50/30 focus:border-indigo-500'
                      }`}
                    />
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'ระดับประสบการณ์ในสายงานขาย/ประกัน' : 'Sales/Insurance Experience'}</label>
                    <select
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'บริษัทเดิม (ถ้ามี)' : 'Previous Company (if any)'}</label>
                    <input
                      type="text"
            ...
            {currentStep === 3 && (
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-medium text-slate-800 mb-2">{language === 'th' ? 'สถานะใบอนุญาตตัวแทนประกันชีวิต *' : 'OIC License Status *'}</label>
                        ...
                        <div
                          ...
                          className={`p-4 rounded-xl border cursor-pointer transition-all ${
                            formData.licenseStatus === item.id
                              ...
                              : 'bg-[#fcfdff] border-sky-50/30 text-slate-800 hover:border-sky-50/30'
                          }`}
                        >
                          <p className="font-semibold text-sm text-slate-800">{item.label}</p>
                          <p className="text-xs text-slate-800 mt-1">{item.desc}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {formData.licenseStatus === 'has_license' && (
                    <>
                      <div>
                        <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'เลขที่ใบอนุญาตตัวแทนประกันชีวิต (คปภ.) *' : 'OIC License Number *'}</label>
                        <input
                          type="text"
                          placeholder="เช่น 6401029384"
                          value={formData.licenseNumber}
                          onChange={(e) => setFormData({ ...formData, licenseNumber: e.target.value })}
                          className="w-full bg-[#fcfdff] border border-sky-50/30/80 rounded-lg px-3 py-2.5 text-sm text-slate-800 font-mono focus:outline-none focus:border-indigo-500"
                        />
                        {validationErrors.licenseNumber && <p className="text-xs text-rose-400 mt-1">{validationErrors.licenseNumber}</p>}
                      </div>

                      <div>
                        <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'วันหมดอายุใบอนุญาต' : 'License Expiry Date'}</label>
                        <input
                          type="date"
                  ...
                  {formData.licenseStatus === 'exam_scheduled' && (
                    <div className="sm:col-span-2">
                      <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'วันที่นัดสอบ คปภ.' : 'Exam Date'}</label>
                      <input
                        type="date"
                ...
                <div className="pt-4 border-t border-sky-50/30">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <label className="flex items-center gap-3 p-3.5 rounded-xl bg-[#fcfdff] border border-sky-50/30/80 cursor-pointer hover:border-sky-50/30">
                      <input
                        type="checkbox"
                        checked={formData.hasICLicense}
                        onChange={(e) => setFormData({ ...formData, hasICLicense: e.target.checked })}
                        className="w-4 h-4 rounded text-indigo-600 focus:ring-indigo-500 bg-[#fcfdff] border-sky-50/30"
                      />
                      <div>
                        ...
                        <p className="text-[11px] text-slate-800">{language === 'th' ? 'สามารถเสนอขายแบบประกันควบการลงทุน (Unit-Linked / Universal Life)' : 'Eligible for Unit-Linked & Wealth products'}</p>
                      </div>
                    </label>
                    ...
                    <label className="flex items-center gap-3 p-3.5 rounded-xl bg-[#fcfdff] border border-sky-50/30/80 cursor-pointer hover:border-sky-50/30">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-800">{language === 'th' ? 'สามารถเสนอขายประกันรถยนต์ พ.ร.บ. และอัคคีภัย' : 'Auto & property insurance selling'}</p>
                      </div>
                    </label>
                  </div>
                </div>
            ...
            {currentStep === 4 && (
              <div className="space-y-6 animate-fadeIn">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {/* Select Sponsor */}
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'ผู้แนะนำเข้าสู่สายงาน (Sponsor) *' : 'Direct Sponsor *'}</label>
                    <select
                  ...
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'เส้นทางเติบโตที่มุ่งหวัง (Career Track)' : 'Preferred Career Track'}</label>
                    <select
                ...
                <div className="p-5 rounded-2xl bg-gradient-to-br from-indigo-950/40 via-sky-100 to-sky-50 border border-indigo-500/30 space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-[#fcfdff] border border-sky-50/30/80 rounded-lg px-3 py-2 text-sm text-blue-600 font-bold focus:outline-none focus:border-blue-600"
                      />
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-[#fcfdff] border border-sky-50/30/80 rounded-lg px-3 py-2 text-sm text-emerald-700 font-bold focus:outline-none focus:border-emerald-500"
                      />
                      <p className="text-[11px] text-slate-800 mt-1">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'ช่องทางที่รู้จัก/แนะนำ' : 'Recruitment Channel'}</label>
                    <select
                  ...
                  <div className="sm:col-span-2">
                    <label className="block text-xs font-medium text-slate-800 mb-1.5">{language === 'th' ? 'ความมุ่งหวัง / เหตุผลที่ตัดสินใจเข้าร่วมธุรกิจ *' : 'Motivation / Why Join *'}</label>
                    <textarea
                      rows={3}
                      ...
                      className={`w-full bg-[#fcfdff] border rounded-lg px-3 py-2 text-sm text-slate-800 focus:outline-none ${
                        validationErrors.motivationNote ? 'border-rose-500' : 'border-sky-50/30 focus:border-indigo-500'
                      }`}
                    />
            ...
            <div className="pt-6 border-t border-sky-50/30 flex items-center justify-between">
              {currentStep > 1 ? (
                <button
                  type="button"
                  onClick={handlePrevStep}
                  className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-sky-50/30 bg-[#f0f9ff] text-slate-800 text-xs font-semibold hover:bg-slate-200 transition-all"
                >
              ...
              {currentStep < 4 ? (
                <button
                  ...
                  className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-blue-600 text-white text-xs font-semibold hover:bg-indigo-500 shadow-[0_1px_3px_rgba(148,163,184,0.08)] shadow-indigo-600/30 transition-all"
                >
                ...
                <button
                  type="submit"
                  className="flex items-center gap-2 px-8 py-3 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 text-slate-900 text-sm font-bold hover:from-emerald-500 hover:to-teal-500 shadow-[0_4px_12px_rgba(148,163,184,0.10)] shadow-emerald-600/30 transition-all"
                >
      ...
      {activeSubTab === 'list' && (
        <div className="space-y-4">
          {/* Filter Bar */}
          <div className="bg-[#fcfdff] border border-sky-50/30/80 rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="relative flex-1 w-full">
              ...
              <input
                ...
                className="w-full bg-[#fcfdff] border border-sky-50/30/80 rounded-lg pl-9 pr-4 py-2 text-sm text-slate-800 focus:outline-none focus:border-indigo-500"
              />
            </div>
            ...
            <div className="flex items-center gap-3 w-full sm:w-auto">
              ...
              <select
                ...
                className="bg-[#fcfdff] border border-sky-50/30/80 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-indigo-500"
              >
          ...
          {/* Applications Table */}
          <div className="bg-[#fcfdff] border border-sky-50/30/80 rounded-xl overflow-hidden shadow-[0_4px_12px_rgba(148,163,184,0.10)]">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm">
                <thead className="bg-[#fcfdff] text-slate-800 uppercase text-[10px] font-bold border-b border-sky-50/30 tracking-wider">
                  <tr>
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {filteredApps.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="px-4 py-12 text-center text-slate-800 text-sm">
                        {language === 'th' ? 'ไม่พบรายการใบสมัครตามเงื่อนไข' : 'No applications found matching criteria'}
                      </td>
                    </tr>
                  ) : (
                    filteredApps.map((app) => (
                      <tr key={app.id} className="hover:bg-[#f0f9ff]/30 transition-colors">
                        <td className="px-4 py-3.5">
                          <p className="font-mono font-bold text-xs text-indigo-700">{app.applicationNo}</p>
                        ...
                        <td className="px-4 py-3.5">
                          <div className="flex items-center gap-2.5">
                            <div className="w-8 h-8 rounded-full bg-[#f0f9ff] flex items-center justify-center font-bold text-xs text-slate-800 border border-sky-50/30">
                            ...
                            <div>
                              ...
                              <p className="text-[11px] text-slate-800 flex items-center gap-1.5 mt-0.5">
                                <Phone className="w-3 h-3 text-slate-800" /> {app.phone} | <MapPin className="w-3 h-3 text-slate-800" /> {app.province}
                              </p>
                            </div>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-3.5">
                          {app.licenseStatus === 'has_license' ? (
                            <div>
                              ...
                              <p className="text-[10px] text-slate-800 font-mono mt-0.5">No. {app.licenseNumber || '-'}</p>
                            </div>
                        ...
                        <td className="px-4 py-3.5">
                          <p className="font-medium text-xs text-slate-800">{app.sponsorName}</p>
                          <span className="text-[10px] text-indigo-700 font-mono">{app.sponsorCode}</span>
                        </td>
                        ...
                        <td className="px-4 py-3.5">
                          ...
                          {app.assignedMemberCode && (
                            <p className="text-[10px] font-mono text-emerald-700 mt-1 font-bold">
                        ...
                        <td className="px-4 py-3.5 text-right">
                          <button
                            ...
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold bg-indigo-600/20 text-indigo-700 border border-indigo-500/30 hover:bg-indigo-600 hover:text-slate-900 transition-all"
                          >
      ...
      {activeSubTab === 'stats' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-[#fcfdff] border border-sky-50/30/80/80 rounded-2xl p-6 shadow-[0_4px_12px_rgba(148,163,184,0.10)] space-y-4">
            ...
            <div className="space-y-3 pt-2">
              {[
                ...
                <div key={idx} className="space-y-1">
                  ...
                  <div className="h-2 bg-[#fcfdff] rounded-full overflow-hidden">
                    <div 
          ...
          <div className="bg-[#fcfdff] border border-sky-50/30/80/80 rounded-2xl p-6 shadow-[0_4px_12px_rgba(148,163,184,0.10)] space-y-4">
            ...
            <div className="p-4 rounded-xl bg-[#fcfdff] border border-sky-50/30/80 space-y-3 text-xs">
              <div className="flex justify-between py-1 border-b border-sky-50/30">
              ...
              <div className="flex justify-between py-1 border-b border-sky-50/30">
                <span className="text-slate-800">{language === 'th' ? 'ผู้สมัครที่มีใบอนุญาต คปภ. แล้ว' : 'Licensed Candidates Ready'}</span>
                <span className="font-bold text-emerald-700">
                  {applications.filter(a => a.licenseStatus === 'has_license').length} {language === 'th' ? 'ท่าน' : 'agents'}
                </span>
              </div>
              <div className="flex justify-between py-1">
                <span className="text-slate-800">{language === 'th' ? 'ผู้สมัครสนใจสายบริหาร Fast-Track' : 'Management Fast-Track Applicants'}</span>
      ...
      {selectedApplication && (
        <div className="fixed inset-0 bg-[#fcfdff]/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-[#fcfdff] border border-sky-50/30/80/80 rounded-2xl max-w-2xl w-full p-6 sm:p-8 space-y-6 shadow-[0_8px_24px_rgba(148,163,184,0.08)] animate-scaleUp max-h-[90vh] overflow-y-auto">
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-sky-50/30 pb-4">
              <div>
                <span className="font-mono text-xs text-indigo-700 font-bold">{selectedApplication.applicationNo}</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
              <div className="p-3.5 rounded-xl bg-[#fcfdff] border border-sky-50/30/80 space-y-1.5">
                ...
                <p className="text-slate-800"><span className="text-slate-800">จังหวัด:</span> {selectedApplication.province} ({selectedApplication.region})</p>
              ...
              <div className="p-3.5 rounded-xl bg-[#fcfdff] border border-sky-50/30/80 space-y-1.5">
                ...
                <p className="text-slate-800"><span className="text-slate-800">ใบอนุญาต คปภ.:</span> {selectedApplication.licenseNumber || selectedApplication.licenseStatus}</p>
                <p className="text-slate-800"><span className="text-slate-800">IC License:</span> {selectedApplication.hasICLicense ? '✅ มี' : '❌ ไม่มี'}</p>
              </div>

              <div className="sm:col-span-2 p-3.5 rounded-xl bg-[#fcfdff] border border-sky-50/30/80 space-y-1.5">
                ...
                <p className="text-slate-800">
                  <span className="text-slate-800">เป้าหมาย FYC:</span> <span className="text-blue-600 font-bold">{(selectedApplication.targetYearlyFYCGoal || 0).toLocaleString()} บาท/ปี</span>
                </p>
                <p className="text-slate-800">
                  <span className="text-slate-800">เป้าหมายรายได้:</span> <span className="text-emerald-700 font-bold">{(selectedApplication.targetMonthlyIncomeGoal || 0).toLocaleString()} บาท/เดือน</span>
                </p>
                <p className="text-slate-800 mt-2 italic bg-[#fcfdff] p-2.5 rounded-lg border border-sky-50/30">
            ...
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full bg-[#fcfdff] border border-sky-50/30/80 rounded-lg p-2.5 text-xs text-slate-800 focus:outline-none focus:border-indigo-500"
              />
            </div>

            {/* Modal Actions */}
            <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-sky-50/30">
              <button
                type="button"
                onClick={() => setSelectedApplication(null)}
                className="px-4 py-2 rounded-xl text-xs font-semibold text-slate-800 hover:text-slate-900 bg-[#f0f9ff]"
              >
                ...
                {selectedApplication.status !== 'approved' && (
                  <>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-5 py-2 rounded-xl text-xs font-bold bg-emerald-600 text-slate-900 hover:bg-emerald-500 shadow-[0_1px_3px_rgba(148,163,184,0.08)] shadow-emerald-600/30 transition-all"
                    >
      ...
      {submissionSuccessModal && (
        <div className="fixed inset-0 bg-[#fcfdff]/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#fcfdff] border border-emerald-500/40 rounded-2xl max-w-md w-full p-6 text-center space-y-4 shadow-[0_8px_24px_rgba(148,163,184,0.08)] animate-scaleUp">
            <div className="w-14 h-14 rounded-full bg-emerald-500/20 border border-emerald-500 text-emerald-700 flex items-center justify-center mx-auto">
            ...
            <div className="p-3 bg-[#fcfdff] rounded-xl border border-sky-50/30 text-xs font-mono text-indigo-700 font-bold">
            ...
            <div className="flex gap-2 pt-2">
              <button
                ...
                className="flex-1 py-2.5 rounded-xl bg-blue-600 text-white text-xs font-semibold hover:bg-indigo-500 transition-all"
              >
              ...
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-[#f0f9ff] text-slate-800 text-xs font-semibold hover:bg-slate-200 transition-all"
              >
