...
export default function InsuranceMegaMenu() {
  ...
  return (
    <header ref={ref} className="relative z-50 border-b border-sky-100 bg-sky-50">
      <nav className="mx-auto flex h-[68px] max-w-7xl items-center justify-between px-5" aria-label="เมนูหลัก">
        ...
        <button onClick={()=>setMobileOpen(true)} className="rounded-lg p-2 text-slate-700 hover:bg-sky-50 lg:hidden"><Menu className="h-6 w-6" /></button>
      </nav>

      {desktopOpen && (
        <>
          <button onClick={()=>setDesktopOpen(false)} className="fixed inset-0 top-[68px] z-40 hidden cursor-default bg-sky-50/20 backdrop-blur-[2px] lg:block" aria-label="ปิดเมนู" />
          <section className="absolute left-1/2 top-[76px] z-50 hidden w-[min(94vw,1200px)] -translate-x-1/2 overflow-hidden rounded-2xl border border-sky-100 bg-sky-50 shadow-2xl lg:block">
            <div className="grid min-h-[480px] grid-cols-[300px_1fr]">
              <aside className="border-r border-sky-100 bg-sky-50 p-6">
                <p className="mb-4 text-[11px] font-bold uppercase tracking-widest text-slate-700">หมวดหมู่</p>
                <div className="space-y-2">
                  {categories.map(c=>{
                    const Icon=c.icon; const sel=c.id===activeId;
                    return (
                      <button key={c.id} onMouseEnter={()=>setActiveId(c.id)} onClick={()=>setActiveId(c.id)} className={`flex w-full items-center gap-3 rounded-xl p-3 text-left transition ${sel ? "bg-blue-600/15 text-blue-700 border border-blue-600/30" : "text-slate-700 hover:bg-sky-50 hover:text-blue-600"}`}>
                        <span className={`grid h-9 w-9 place-items-center rounded-full ${sel ? "bg-blue-600 text-slate-950" : "bg-sky-50 text-slate-700"}`}><Icon className="h-5 w-5" /></span>
              ...
              <main className="p-7 bg-sky-50">
                <div className="relative mb-6 overflow-hidden rounded-2xl bg-gradient-to-r from-sky-50 via-sky-100/40 to-indigo-600 p-7 text-white border border-blue-600/20">
                  <div className="relative z-10 max-w-xl">
                    ...
                    <p className="mt-2 text-sm leading-6 text-slate-700">{active.description}</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {active.links.map(l=>{
                    const Icon=l.icon;
                    return (
                      <button key={l.tab} onClick={()=>go(l.tab)} className="group flex gap-3 rounded-xl border border-sky-100 bg-sky-50 p-4 text-left transition hover:border-blue-600/40 hover:bg-sky-50">
                        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-sky-50 text-slate-700 group-hover:bg-blue-600 group-hover:text-slate-950 transition"><Icon className="h-5 w-5" /></span>
                        <span className="min-w-0"><span className="flex items-center gap-1 text-[13px] font-bold text-slate-900 group-hover:text-blue-600">{l.title} {l.badge && <span className="ml-1 rounded-full bg-blue-600 px-1.5 py-0.5 text-[9px] font-black text-slate-950">{l.badge}</span>}<ChevronRight className="h-3.5 w-3.5 opacity-0 group-hover:opacity-100 group-hover:translate-x-0.5 transition" /></span><span className="mt-1 block text-xs leading-5 text-slate-700">{l.description}</span></span>
                      </button>
                    );
                  })}
                </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-[70] bg-sky-50 lg:hidden">
          <div className="flex h-[68px] items-center justify-between border-b border-sky-100 px-5">
            <span className="text-[15px] font-black text-slate-900">AI INSURANCE NETWORK OS</span>
            <button onClick={()=>setMobileOpen(false)} className="rounded-lg p-2 text-slate-700 hover:bg-sky-50"><X className="h-6 w-6" /></button>
          </div>
          <div className="h-[calc(100vh-68px)] overflow-y-auto p-5 space-y-3">
            {categories.map(c=>{
              const Icon=c.icon; const exp=activeId===c.id;
              return (
                <div key={c.id} className="overflow-hidden rounded-xl border border-sky-100 bg-sky-50">
                  <button onClick={()=>setActiveId(exp ? "" : c.id)} className="flex w-full items-center gap-3 p-4 text-left">
                    ...
                    <ChevronDown className={`h-5 w-5 text-slate-700 transition-transform ${exp ? "rotate-180" : ""}`} />
                  </button>
                  {exp && (
                    <div className="space-y-2 border-t border-sky-100 bg-sky-50 p-3">
                      {c.links.map(l=>{
                        const Ic=l.icon;
                        return (
                          <button key={l.tab} onClick={()=>go(l.tab)} className="flex w-full gap-3 rounded-lg bg-sky-50 p-3 text-left border border-sky-100">
                            <Ic className="mt-0.5 h-5 w-5 shrink-0 text-blue-600" />
                            <span><span className="block text-[13px] font-bold text-slate-900">{l.title}</span><span className="mt-1 block text-xs text-slate-700">{l.description}</span></span>
                          </button>
                        );
                      })}
                    </div>
