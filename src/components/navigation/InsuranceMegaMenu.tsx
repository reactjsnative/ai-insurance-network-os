          ...
          <section className="absolute left-1/2 top-[76px] z-50 hidden w-[min(94vw,1200px)] -translate-x-1/2 overflow-hidden rounded-2xl border border-sky-50/40 bg-white shadow-[0_8px_24px_rgba(148,163,184,0.08)] lg:block">
            <div className="grid min-h-[480px] grid-cols-[300px_1fr]">
              <aside className="border-r border-sky-50/30/40 bg-[#f8fbff] p-6">
                ...
                <div className="space-y-2">
                  {categories.map(c=>{
                    ...
                    return (
                      <button key={c.id} onMouseEnter={()=>setActiveId(c.id)} onClick={()=>setActiveId(c.id)} className={`flex w-full items-center gap-3 rounded-xl p-3 text-left transition ${sel ? "bg-blue-50 text-blue-700 border border-blue-200" : "text-slate-800 hover:bg-[#fcfdff] hover:text-blue-700"}`}>
                        <span className={`grid h-9 w-9 place-items-center rounded-full ${sel ? "bg-blue-600 text-white" : "bg-white text-slate-800"}`}><Icon className="h-5 w-5" /></span>
              ...
              <main className="p-7 bg-white">
                <div className="relative mb-6 overflow-hidden rounded-2xl bg-gradient-to-r from-[#fcfdff] via-[#f8fbff] to-[#e0f2fe]/20 p-7 text-slate-900 border border-sky-50/30">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {active.links.map(l=>{
                    const Icon=l.icon;
                    return (
                      <button key={l.tab} onClick={()=>go(l.tab)} className="group flex gap-3 rounded-xl border border-sky-50/40 bg-white p-4 text-left transition hover:border-blue-600/40 hover:bg-white">
                        <span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-[#fcfdff] text-slate-800 group-hover:bg-blue-600 group-hover:text-white transition"><Icon className="h-5 w-5" /></span>
        ...
        <div className="fixed inset-0 z-[70] bg-white lg:hidden">
          ...
          <div className="h-[calc(100vh-68px)] overflow-y-auto p-5 space-y-3">
            {categories.map(c=>{
              ...
              return (
                <div key={c.id} className="overflow-hidden rounded-xl border border-sky-50/40 bg-white">
                  ...
                  {exp && (
                    <div className="space-y-2 border-t border-sky-50/40 bg-white p-3">
                      {c.links.map(l=>{
                        const Ic=l.icon;
                        return (
                          <button key={l.tab} onClick={()=>go(l.tab)} className="flex w-full gap-3 rounded-lg bg-white p-3 text-left border border-sky-50/40">
