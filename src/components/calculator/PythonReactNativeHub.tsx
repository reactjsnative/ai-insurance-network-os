    ...
    <div className="p-4 sm:p-6 bg-white text-slate-900 font-sans space-y-4 text-left rounded-3xl border border-slate-200 shadow-xl">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <p className="text-[10px] text-slate-600">อิงโครงสร้างผลตอบแทน 13 รายการ 4 ระดับตำแหน่ง ปรับปรุง 15 ม.ค. 64</p>
        </div>
      </div>
        ...
        {[
          ...
          <button
            ...
            className={`py-1.5 px-1 rounded-lg text-center transition-all cursor-pointer ${
              position === p.id ? 'bg-amber-500 text-slate-950 font-bold shadow' : 'text-slate-600 hover:text-slate-800'
            }`}
          >
      ...
      <div className="p-3.5 rounded-2xl bg-white/90 border border-slate-200 space-y-2.5">
        ...
        <div className="flex items-center justify-between text-xs">
          <span className="text-slate-600 text-[11px]">บำเหน็จส่วนตัว (COM):</span>
          <input
            type="number"
            value={personalCom}
            onChange={(e) => setPersonalCom(Number(e.target.value) || 0)}
            className="w-24 bg-white border border-slate-200 rounded-lg px-2 py-1 text-right text-xs text-amber-400 font-bold"
          />
        </div>

        {position !== 'agent' && (
          <div className="flex items-center justify-between text-xs">
            <span className="text-slate-600 text-[11px]">COM รวมทั้งทีม:</span>
            <input
              type="number"
              value={teamCom}
        ...
        {position === 'region_manager' && (
          <div className="flex items-center justify-between text-xs">
            <span className="text-slate-600 text-[11px]">FYC รวมทั้งทีม (ภาค):</span>
            <input
              type="number"
              value={teamFyc}
        ...
        {position !== 'agent' && (
          <div className="flex items-center justify-between text-xs">
            <span className="text-slate-600 text-[11px]">จำนวนหน่วยแยก:</span>
            <input
              type="number"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-5 rounded-3xl bg-white/90 border border-slate-200 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-mono">โครงสร้าง & คุณสมบัติ</span>
            </div>
            ...
            <div className="space-y-2 text-xs text-slate-700">
              <div className="p-3 rounded-xl bg-white border border-slate-200/80 space-y-1">
                ...
                <p className="text-slate-600 text-[11px]">บำเหน็จ 20,000 บาท (เวลา 1-6 เดือน)</p>
              </div>
              <div className="p-3 rounded-xl bg-white border border-slate-200/80 space-y-1">
                <div className="font-bold text-emerald-300">2. ผู้บริหารหน่วย (UM) → ผู้บริหารศูนย์ (CM)</div>
                <p className="text-slate-600 text-[11px]">บำเหน็จ 75,000 บาท (เวลา 3-6 เดือน) + แยกหน่วย 2 หน่วย</p>
              </div>
              <div className="p-3 rounded-xl bg-white border border-slate-200/80 space-y-1">
                <div className="font-bold text-amber-300">3. ผู้บริหารศูนย์ (CM) → ผู้บริหารภาค (RM)</div>
                <p className="text-slate-600 text-[11px]">บำเหน็จ 1,200,000 บาท (เวลา 12-24 เดือน) + แยกศูนย์ 4 ศูนย์</p>
              </div>
            </div>
          </div>
          ...
          <div className="p-5 rounded-3xl bg-white/90 border border-slate-200 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-mono">UM Rules</span>
            </div>
          ...
          <div className="p-5 rounded-3xl bg-white/90 border border-slate-200 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-mono">CM Rules</span>
            </div>
          ...
          <div className="p-5 rounded-3xl bg-white/90 border border-slate-200 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-mono">RM Rules</span>
            </div>
