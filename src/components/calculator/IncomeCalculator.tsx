...
export const IncomeCalculator: React.FC = () => {
  ...
  const getPositionBadge = (posId: string) => {
    switch (posId) {
      ...
      default: return { label: 'ตัวแทน (Agent)', bg: 'bg-[#fcfdff]0/20 text-sky-300 border-sky-500/40' };
    }
  };
  ...
  return (
    <div id="income_calculator_view" className="space-y-6 max-w-7xl mx-auto pb-16 text-left">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-3xl bg-[#fcfdff]/90 border border-sky-50/40 shadow-[0_4px_12px_rgba(148,163,184,0.10)]">
        <div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="inline-flex p-1 rounded-xl bg-[#fcfdff] border border-sky-50/40">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                calcMode === 'ACTUAL' 
                  ? 'bg-blue-600 text-slate-950 shadow-[0_1px_3px_rgba(148,163,184,0.08)] shadow-amber-500/20' 
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                calcMode === 'PROJECTED' 
                  ? 'bg-[#fcfdff]0 text-slate-950 shadow-[0_1px_3px_rgba(148,163,184,0.08)] shadow-sky-500/20' 
                  : 'text-slate-800 hover:text-slate-800'
              }`}
            >
              PROJECTED (คาดการณ์)
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                calcMode === 'SIMULATION' 
                  ? 'bg-purple-500 text-slate-950 shadow-[0_1px_3px_rgba(148,163,184,0.08)] shadow-purple-500/20' 
                  : 'text-slate-800 hover:text-slate-800'
              }`}
            >
          ...
          <button
            ...
            className="px-3 py-2 rounded-xl bg-[#f0f9ff] hover:bg-slate-200 text-slate-800 text-xs font-semibold border border-sky-50/40 flex items-center gap-1.5 transition-colors"
          >
      ...
      {calcMode !== 'ACTUAL' && (
        <div className="p-5 sm:p-6 rounded-2xl bg-[#fcfdff]/60 border border-purple-500/30 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="text-[10px] text-slate-800">คำนวณและอัปเดตผลตอบแทนทันทีแบบเรียลไทม์</span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {/* Personal COM */}
            <div className="p-3 rounded-xl bg-[#fcfdff] border border-sky-50/40">
            ...
            {/* Position Selector */}
            <div className="p-3 rounded-xl bg-[#fcfdff] border border-sky-50/40">
              <div className="text-xs text-slate-800 mb-1.5">ตำแหน่งที่ใช้จำลอง (Simulated Rank)</div>
              <select
                value={customParams.positionId}
                onChange={(e) => setCustomParams({ ...customParams, positionId: e.target.value })}
                className="w-full bg-[#fcfdff] border border-sky-50/40 rounded-lg px-2 py-1 text-xs text-slate-800 focus:outline-none"
              >
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div>
            ...
            <div className="text-3xl sm:text-5xl font-black text-slate-900 tracking-tight mt-2 flex items-baseline gap-2">
              ...
              <span className="text-sm sm:text-base font-medium text-slate-800">/ เดือน</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="p-3 rounded-2xl bg-[#fcfdff]/80 border border-sky-50/40">
              <span className="text-slate-800 block text-[10px]">บำเหน็จส่วนตัว (Direct)</span>
            ...
            <div className="p-3 rounded-2xl bg-[#fcfdff]/80 border border-sky-50/40">
              <span className="text-slate-800 block text-[10px]">ผลตอบแทนบริหารทีม (Override)</span>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-[#f0f9ff] text-slate-800 border border-sky-50/40">
              {incomeResult.breakdown.length} รายการที่เข้าเกณฑ์
            </span>
          </div>
          <span className="text-[11px] text-slate-800">คลิกการ์ดเพื่อดูสูตรคำนวณและเกณฑ์ขั้นบันได</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {incomeResult.breakdown.map((item) => (
            <div
              ...
              className="p-4 rounded-2xl bg-[#fcfdff]/80 hover:bg-[#fcfdff] border border-sky-50/40 hover:border-blue-600/50 cursor-pointer transition-all duration-200 group flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${
                    item.amount > 0 ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-500/30' : 'bg-[#f0f9ff] text-slate-800'
                  }`}>
                ...
                <div className="text-[11px] text-slate-800 mt-1 line-clamp-2 leading-relaxed">
                  {item.description}
                </div>
              </div>

              <div className="pt-3 mt-3 border-t border-sky-50/40/80 flex items-center justify-between text-[10px] text-slate-800">
      ...
      {selectedIncomeCard && (
        <div className="fixed inset-0 bg-[#fcfdff]/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#fcfdff] border border-sky-50/40 rounded-3xl max-w-lg w-full p-6 shadow-[0_8px_24px_rgba(148,163,184,0.08)] space-y-4 animate-in fade-in zoom-in duration-200">
            <div className="flex items-center justify-between pb-3 border-b border-sky-50/40">
              ...
              <button
                ...
                className="w-7 h-7 rounded-lg bg-[#f0f9ff] text-slate-800 hover:text-slate-900 flex items-center justify-center text-xs"
              >
                ✕
              </button>
            </div>
            ...
            <div className="space-y-3">
              <div className="p-3 rounded-xl bg-[#fcfdff] border border-sky-50/40">
                <span className="text-[10px] text-slate-800 uppercase font-bold">ยอดเงินที่คำนวณได้</span>
              ...
              <div className="p-3 rounded-xl bg-[#fcfdff] border border-sky-50/40">
                <span className="text-[10px] text-slate-800 uppercase font-bold">อัตราและสูตรที่ใช้</span>
              ...
              <div className="p-3 rounded-xl bg-[#fcfdff] border border-sky-50/40">
                <span className="text-[10px] text-slate-800 uppercase font-bold">หลักเกณฑ์และเงื่อนไข</span>
            ...
            <button
              onClick={() => setSelectedIncomeCard(null)}
              className="w-full py-2.5 rounded-xl bg-[#f0f9ff] hover:bg-slate-200 text-slate-800 text-xs font-bold transition-colors"
            >
              ปิดหน้าต่าง
            </button>
          </div>
      ...
      {showTestModal && (
        <div className="fixed inset-0 bg-[#fcfdff]/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#fcfdff] border border-sky-50/40 rounded-3xl max-w-xl w-full p-6 shadow-[0_8px_24px_rgba(148,163,184,0.08)] space-y-4 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-sky-50/40">
              <div className="flex items-center gap-2">
            ...
            <div className="space-y-2">
              {testResults.map((t, idx) => (
                <div key={idx} className="p-3 rounded-xl bg-[#fcfdff] border border-sky-50/40 flex items-center justify-between text-xs">
                  <div>
