...
export const IncomeCalculator: React.FC = () => {
  ...
  return (
    <div id="income_calculator_view" className="space-y-6 max-w-7xl mx-auto pb-16 text-left">
      ...
      {calcMode !== 'ACTUAL' && (
        <div className="p-5 sm:p-6 rounded-2xl bg-slate-900/60 border border-purple-500/30 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            {/* Position Selector */}
            <div className="p-3 rounded-xl bg-slate-950 border border-slate-800">
              <div className="text-xs text-slate-400 mb-1.5">ตำแหน่งที่ใช้จำลอง (Simulated Rank)</div>
              <select
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-amber-500/10 via-slate-900 to-indigo-950/40 border border-amber-500/30 p-6 sm:p-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div>
            ...
            <div className="text-3xl sm:text-5xl font-black text-slate-100 tracking-tight mt-2 flex items-baseline gap-2">
              <span>฿{incomeResult.totalIncome.toLocaleString()}</span>
              <span className="text-sm sm:text-base font-medium text-slate-400">/ เดือน</span>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="p-3 rounded-2xl bg-slate-950/80 border border-slate-800">
              <span className="text-slate-400 block text-[10px]">บำเหน็จส่วนตัว (Direct)</span>
              <span className="font-bold text-slate-200 text-sm">฿{incomeResult.breakdown.find(b => b.id.includes('direct') || b.id.includes('personal'))?.amount.toLocaleString() || '0'}</span>
            </div>
            <div className="p-3 rounded-2xl bg-slate-950/80 border border-slate-800">
              <span className="text-slate-400 block text-[10px]">ผลตอบแทนบริหารทีม (Override)</span>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[11px] text-slate-400">คลิกการ์ดเพื่อดูสูตรคำนวณและเกณฑ์ขั้นบันได</span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {incomeResult.breakdown.map((item) => (
            <div
              ...
              className="p-4 rounded-2xl bg-slate-900/80 hover:bg-slate-900 border border-slate-800 hover:border-amber-500/50 cursor-pointer transition-all duration-200 group flex flex-col justify-between"
            >
              <div>
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${
                    item.amount > 0 ? 'bg-emerald-500/10 text-emerald-300 border border-emerald-500/30' : 'bg-slate-800 text-slate-500'
                  }`}>
      ...
      {selectedIncomeCard && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4 animate-in fade-in zoom-in duration-200">
            ...
            <div className="space-y-3">
              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800">
                <span className="text-[10px] text-slate-400 uppercase font-bold">ยอดเงินที่คำนวณได้</span>
              ...
              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800">
                <span className="text-[10px] text-slate-400 uppercase font-bold">อัตราและสูตรที่ใช้</span>
              ...
              <div className="p-3 rounded-xl bg-slate-950 border border-slate-800">
                <span className="text-[10px] text-slate-400 uppercase font-bold">หลักเกณฑ์และเงื่อนไข</span>
      ...
      {showTestModal && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-3xl max-w-xl w-full p-6 shadow-2xl space-y-4 max-h-[85vh] overflow-y-auto">
            <div className="flex items-center justify-between pb-3 border-b border-slate-800">
              <div className="flex items-center gap-2">
            ...
            <div className="space-y-2">
              {testResults.map((t, idx) => (
                <div key={idx} className="p-3 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between text-xs">
                  <div>
                    <div className="font-semibold text-slate-200">{t.testName}</div>
