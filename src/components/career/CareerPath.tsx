  ...
  return (
    <div id="career_path_view" className="space-y-6 max-w-7xl mx-auto pb-16 text-left">
        ...
        <div className="inline-flex p-1 rounded-xl bg-slate-950 border border-slate-800 self-start md:self-auto">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
              selectedTargetRank === 'unit_manager' ? 'bg-emerald-500 text-slate-950' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            ผู้บริหารหน่วย (UM)
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
              selectedTargetRank === 'center_manager' ? 'bg-amber-500 text-slate-950' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            ผู้บริหารศูนย์ (CM)
          </button>
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
              selectedTargetRank === 'region_manager' ? 'bg-rose-500 text-slate-950' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            ผู้บริหารภาค (RM)
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="p-6 rounded-3xl bg-gradient-to-br from-slate-900 to-indigo-950/40 border border-slate-800 flex flex-col justify-between">
          <div>
            <span className="text-xs font-bold text-amber-400 uppercase tracking-wider">
              เป้าหมาย: {targetCriteria.title}
            </span>
            <div className="text-4xl font-black text-slate-100 mt-2 font-mono">
              {overallScore}% <span className="text-xs font-normal text-slate-400">ความพร้อม</span>
            </div>
          ...
          <div className="space-y-3 mt-6 pt-4 border-t border-slate-800">
            <div className="flex justify-between text-xs">
              <span className="text-slate-400">กรอบเวลาสะสม:</span>
              <span className="font-bold text-slate-200">{targetCriteria.timeframeMonths}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-slate-400">เกณฑ์ดำรงตำแหน่งรายปี:</span>
        ...
        <div className="lg:col-span-2 p-6 rounded-3xl bg-slate-900/90 border border-slate-800 space-y-5">
          ...
          <div className="space-y-1.5">
            <div className="flex justify-between text-xs">
              <span className="font-semibold text-slate-300">1. บำเหน็จสะสม (COM Requirement)</span>
          ...
          {targetCriteria.unitsRequired > 0 && (
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                <span className="font-semibold text-slate-300">2. การแตกหน่วยงาน (Separated Units)</span>
                <span className="font-mono text-slate-400">
          ...
          {targetCriteria.centersRequired > 0 && (
            <div className="space-y-1.5">
              <div className="flex justify-between text-xs">
                <span className="font-semibold text-slate-300">3. การแตกศูนย์งาน (Separated Centers)</span>
                <span className="font-mono text-slate-400">
