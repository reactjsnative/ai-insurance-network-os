    ...
    <div id="career_plan_view" className="space-y-8 max-w-7xl mx-auto pb-16 text-left">
      ...
      <div className="p-6 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-amber-500 to-rose-600 flex items-center justify-center text-slate-100 shadow-lg shadow-amber-500/30">
        ...
        <div className="p-5 rounded-2xl bg-gradient-to-r from-indigo-500/15 to-fuchsia-500/10 border border-indigo-500/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <h3 className="text-lg font-black text-slate-100">{activeUser.name}</h3>
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-[11px] text-slate-400">สมาชิก {memberDownline?.totalDownlineCount ?? 0} คน • COM ส่วนตัว {fmt(activeUser.personalCOM)}</span>
              </div>
            </div>
          </div>
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-fuchsia-500 hover:from-indigo-400 hover:to-fuchsia-400 text-white text-xs font-bold shadow-lg shadow-indigo-500/20 transition-all whitespace-nowrap"
          >
            เริ่มสร้างแผนของฉัน →
          </button>
        </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 px-1">
          ...
          <h2 className="text-lg font-black text-slate-100">1. ตั้งค่าข้อมูลของคุณ (Smart Onboarding)</h2>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-2">
            <label className="text-xs font-semibold text-slate-300 block">Retention / Activation</label>
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-slate-400 w-16">Retain {(retention * 100).toFixed(0)}%</span>
            ...
            <div className="flex items-center gap-2">
              <span className="text-[10px] text-slate-400 w-16">Active {(activation * 100).toFixed(0)}%</span>
      ...
      <section className="space-y-4">
        ...
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-300">ความคืบหน้าสู่ {POSITION_LABELS[targetPosition]}</span>
          ...
          <p className="text-[11px] text-slate-400 leading-relaxed">{career.mathematicalProjection.recommendationText}</p>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 px-1">
          ...
          <h2 className="text-lg font-black text-slate-100">3. คำนวณย้อนหลัง & วิเคราะห์ช่องว่าง (Reverse + Gap)</h2>
        </div>
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-fuchsia-950/20 border border-slate-800 space-y-4">
            <div className="flex items-center gap-2 text-fuchsia-300">
              <Target className="w-4 h-4" />
              <span className="text-xs font-bold uppercase tracking-wider">Reverse Goal Calculator</span>
            </div>
            <p className="text-xs text-slate-300">เป้าหมายรายได้ {fmt(targetIncome)}/เดือน ภายใน {targetMonths} เดือน</p>
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-amber-950/20 border border-slate-800 space-y-4">
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 px-1">
          ...
          <h2 className="text-lg font-black text-slate-100">4. จำลองสถานการณ์ (What-If Simulator)</h2>
        </div>
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-slate-300">ส่วนประกอบรายได้จำลอง</h3>
            <div className="h-48">
            ...
            <p className="text-[11px] text-slate-400">รวมรายได้จำลอง: <b className="text-emerald-300">{fmt(incomeResult.totalIncome)}/เดือน</b></p>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 px-1">
          ...
          <h2 className="text-lg font-black text-slate-100">5. ประมาณการอนาคต (Forecast 3 เส้น)</h2>
        </div>
        <div className="p-5 rounded-2xl bg-slate-900/60 border border-slate-800">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-4">
            {forecast.map((s) => {
              ...
              return (
                <div key={s.key} className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                  <div className="flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full" style={{ background: s.color }} />
                    <span className="text-xs font-bold text-slate-200">{s.label}</span>
                  </div>
                  <p className="text-[11px] text-slate-400 mt-1">เดือน 12: {fmt(m12?.projectedMonthlyIncome || 0)}</p>
                  <p className="text-[11px] text-slate-400">เดือน 36: {fmt(m36?.projectedMonthlyIncome || 0)}</p>
                </div>
              );
            })}
          </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 px-1">
          ...
          <h2 className="text-lg font-black text-slate-100">6. AI Team Coach (คำแนะนำอัตโนมัติ)</h2>
        </div>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-2 px-1">
          ...
          <h2 className="text-lg font-black text-slate-100">7. แผนผังการขยายเครือข่ายแบบไม่มีที่สิ้นสุด (Infinite Network Tree)</h2>
      ...
      {memberMode && (
        <section className="space-y-4">
          <div className="flex items-center gap-2 px-1">
            ...
            <h2 className="text-lg font-black text-slate-100">8. วิดีโอสรุปแผนของฉัน</h2>
          ...
          <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-fuchsia-950/20 border border-slate-800 space-y-4">
            ...
            <button
              ...
              className="px-5 py-3 rounded-xl bg-gradient-to-r from-fuchsia-500 to-purple-500 hover:from-fuchsia-400 hover:to-purple-400 text-white text-xs font-bold shadow-lg shadow-fuchsia-500/20 transition-all disabled:opacity-60 flex items-center gap-2"
            >
              {videoLoading ? (
                <>
            ...
            {videoUrl && (
              <div className="rounded-2xl overflow-hidden border border-fuchsia-500/30 bg-slate-950">
...
const KpiCard: React.FC<{ label: string; value: string; sub: string; accent: 'sky' | 'violet' | 'emerald' | 'amber' }> = ({ label, value, sub, accent }) => {
  ...
  return (
    <div className={`p-4 rounded-2xl bg-gradient-to-br to-slate-950 border border-slate-800 ${ring}`}>
...
const GapPill: React.FC<{ label: string; value: string }> = ({ label, value }) => (
  <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
    ...
    <p className="text-sm font-bold text-slate-100 mt-0.5">{value}</p>
  </div>
);
...
const InfiniteNetworkTree: React.FC<{
  ...
  return (
    <div className="p-5 rounded-2xl bg-gradient-to-br from-slate-900 to-rose-950/10 border border-slate-800 space-y-4">
      {/* Toolbar */}
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-300 font-mono">{depth} ชั้น</span>
          <button
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
          <p className="text-[10px] text-slate-400">จำนวนคนในแผนผัง</p>
          <p className="text-sm font-black text-rose-300">{totalPeople.toLocaleString()} คน</p>
        </div>
        <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
          <p className="text-[10px] text-slate-400">ศูนย์สูงสุดที่เป็นไปได้</p>
          <p className="text-sm font-black text-amber-300">~{Math.floor(totalPeople / 15)} ศูนย์</p>
        </div>
        <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
          <p className="text-[10px] text-slate-400">หน่วยสูงสุดที่เป็นไปได้</p>
          <p className="text-sm font-black text-cyan-300">~{Math.floor(totalPeople / 5)} หน่วย</p>
        </div>
        <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
          <p className="text-[10px] text-slate-400">Potential COM สะสม</p>
          <p className="text-sm font-black text-emerald-300">{fmt(totalPotential)}</p>
        </div>
      </div>

      {/* Tree (vertical layers) */}
      <div className="relative rounded-2xl bg-slate-950/70 border border-slate-800 p-4 overflow-x-auto">
            ...
            <div key={layer.gen} className="flex flex-col items-center">
                ...
                {layer.nodes.map((n) => {
                  ...
                  return (
                    <button
                      ...
                      className={`group relative flex flex-col items-center p-2 rounded-xl border bg-slate-900/80 transition-all hover:-translate-y-0.5 ${
                        isSel ? `${role.ring} ring-2 ring-rose-400/40` : 'border-slate-800 hover:border-slate-600'
                      }`}
                    ...
                    >
                      ...
                      />
                      <span className="text-[9px] text-slate-300 mt-1 max-w-[64px] truncate">{n.name}</span>
                    </button>
                  );
                })}
              </div>
              {li < layers.length - 1 && (
                <div className="flex items-center gap-1 text-slate-600 my-1">
                  ...
                  <span className="text-[9px] font-mono text-slate-500">แตก {branching}×</span>
                  <ChevronRight className="w-3 h-3" />
                  <div className="w-8 h-px bg-slate-700" />
                </div>
              )}
      ...
      {selectedNode && (
        <div className="p-4 rounded-xl bg-rose-500/10 border border-rose-500/30 flex items-center gap-3">
          ...
          <div className="text-xs text-slate-200">
            ...
            <span className="text-slate-400"> — คลิกโหนดอื่นเพื่อดูรายละเอียด (จำลองเส้นทางสายงาน)</span>
          </div>
        </div>
      )}
