...
export const AICoach: React.FC = () => {
  ...
  return (
    <div id="ai_coach_view" className="space-y-6 max-w-7xl mx-auto pb-16 text-left">
      ...
      <div className="rounded-3xl bg-sky-50/90 border border-sky-100 shadow-2xl flex flex-col h-[520px] overflow-hidden">
        {/* Chat Messages Log */}
        <div className="flex-1 p-4 sm:p-6 overflow-y-auto space-y-4">
              ...
              <div
                className={`max-w-xl rounded-2xl p-4 text-xs sm:text-sm leading-relaxed space-y-1 ${
                  msg.sender === 'user'
                    ...
                    : 'bg-sky-50/80 border border-sky-100 text-slate-800 rounded-tl-none'
                }`}
              >
          ...
          {loading && (
            <div className="flex gap-3 justify-start">
              ...
              <div className="bg-sky-50/80 border border-sky-100 rounded-2xl rounded-tl-none p-4 text-xs text-slate-700 flex items-center gap-2">
        ...
        <div className="p-3 bg-sky-50/60 border-t border-sky-100/80 flex items-center gap-2 overflow-x-auto">
          ...
          {quickPrompts.map((p, idx) => (
            <button
              ...
              disabled={loading}
              className="px-3 py-1.5 rounded-full bg-sky-50 hover:bg-sky-100 text-slate-700 border border-sky-100/80 text-xs whitespace-nowrap transition-colors"
            >
              {p}
            </button>
          ))}
        ...
        {/* Chat Input Bar */}
        <div className="p-4 bg-sky-50 border-t border-sky-100 flex items-center gap-2">
          <input
            type="text"
            ...
            className="flex-1 bg-sky-50 border border-sky-100 rounded-xl px-4 py-2.5 text-xs sm:text-sm text-slate-800 placeholder-slate-500 focus:outline-none focus:border-blue-600/50"
          />
          <button
