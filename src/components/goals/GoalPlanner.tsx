      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 border border-indigo-500/20 p-6 sm:p-8 shadow-xl">
          ...
          <div className="flex items-center gap-2 bg-slate-950 p-1.5 rounded-xl border border-slate-800 self-start md:self-auto">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                selectedScenario === 'conservative' ? 'bg-slate-700 text-white' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                selectedScenario === 'standard' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-600/30' : 'text-slate-400 hover:text-white'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                selectedScenario === 'aggressive' ? 'bg-amber-600 text-white shadow-lg shadow-amber-600/30' : 'text-slate-400 hover:text-white'
              }`}
            >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-6 border-t border-slate-800/80">
          <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/60">
            ...
            <p className="text-2xl font-black text-white mt-1">{formatNumber(targetActiveMembers)} <span className="text-xs text-slate-400 font-normal">คน</span></p>
            <p className="text-[11px] text-slate-400 mt-1">ในระยะเวลา {targetTimeMonths} เดือน</p>
          </div>

          <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/60">
            <p className="text-xs text-amber-400 font-medium">{language === 'th' ? 'อัตราสรรหาที่ต้องทำ' : 'Required Recruits/Mo'}</p>
            <p className="text-2xl font-black text-amber-400 mt-1">{calculation.monthlyRecruitsNeeded} <span className="text-xs text-slate-400 font-normal">คน/เดือน</span></p>
            <p className="text-[11px] text-slate-400 mt-1">รวมทั้งสิ้น {formatNumber(calculation.totalRecruitsNeeded)} คน</p>
          </div>

          <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/60">
            <p className="text-xs text-cyan-400 font-medium">{language === 'th' ? 'โครงสร้างผู้นำที่ต้องสร้าง' : 'Leadership Blueprint'}</p>
            <p className="text-2xl font-black text-cyan-400 mt-1">{calculation.estimatedUnits} <span className="text-xs text-slate-400 font-normal">หน่วย</span> / {calculation.estimatedCenters} <span className="text-xs text-slate-400 font-normal">ศูนย์</span></p>
            <p className="text-[11px] text-slate-400 mt-1">สร้าง {calculation.estimatedRegions} ภาคใหญ่</p>
          </div>
          ...
          <div className="bg-slate-950/60 p-3.5 rounded-xl border border-slate-800/60">
            ...
            <p className="text-[11px] text-slate-400 mt-1">~{formatCurrency(calculation.projectedAnnualFYC)} / ปี</p>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-4">
            <div>
              <div className="flex items-center justify-between text-xs mb-1.5">
                <label className="text-slate-300 font-medium">{language === 'th' ? 'เป้าหมายสมาชิก Active สิ้นสุด' : 'Target Active Members'}</label>
            ...
            <div>
              <div className="flex items-center justify-between text-xs mb-1.5">
                <label className="text-slate-300 font-medium">{language === 'th' ? 'กรอบเวลาเป้าหมาย (เดือน)' : 'Timeline Duration (Months)'}</label>
                <span className="font-bold text-amber-400">{targetTimeMonths} เดือน ({Math.round(targetTimeMonths/12 * 10)/10} ปี)</span>
              </div>
              <input
                type="range"
                min={6}
            ...
            <div>
              <div className="flex items-center justify-between text-xs mb-1.5">
                <label className="text-slate-300 font-medium">{language === 'th' ? 'สมาชิก Active ปัจจุบัน' : 'Current Active Baseline'}</label>
                <span className="font-bold text-slate-200">{currentActiveMembers} คน</span>
              </div>
              <input
                type="number"
            ...
            <div className="pt-3 border-t border-slate-800 space-y-4">
              <div>
                <div className="flex items-center justify-between text-xs mb-1.5">
                  <label className="text-slate-300 font-medium">{language === 'th' ? 'อัตราคงอยู่รายเดือน (Monthly Retention)' : 'Monthly Retention Rate'}</label>
                ...
                />
                <p className="text-[11px] text-slate-400 mt-1">Churn Rate: {100 - monthlyRetentionRate}% ต่อเดือน</p>
              </div>

              <div>
                <div className="flex items-center justify-between text-xs mb-1.5">
                  <label className="text-slate-300 font-medium">{language === 'th' ? 'อัตราเริ่มงานจริง (Activation Rate)' : 'Activation Rate'}</label>
              ...
              <div>
                <div className="flex items-center justify-between text-xs mb-1.5">
                  <label className="text-slate-300 font-medium">{language === 'th' ? 'FYC เฉลี่ยต่อคนต่อเดือน (บาท)' : 'Avg FYC per Active Member'}</label>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              ...
              <span className="text-xs text-slate-400">{calculation.milestones.length} หมุดหมายสำคัญ</span>
            </div>
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-800/60">
                  {calculation.milestones.map((m) => (
                    <tr key={m.step} className="hover:bg-slate-800/40 transition-all">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-2">
              ...
              <p className="text-xs text-slate-300">
                ต้องรักษาอัตราการรับสมัครใหม่อย่างน้อย <strong className="text-amber-400">{calculation.monthlyRecruitsNeeded} คน/เดือน</strong> ทั่วทั้งองค์กร หรือเฉลี่ย 1 คนต่อเดือนสำหรับตัวแทนหลัก
              </p>
            </div>
