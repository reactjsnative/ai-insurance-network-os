...
export const OrganizationStructure: React.FC = () => {
  ...
  const getPositionBadge = (posId: PositionId) => {
    switch (posId) {
      ...
      default:
        return <span className="px-2 py-0.5 rounded text-[11px] font-medium bg-[#f0f9ff] text-slate-800 border border-sky-50/40">ตัวแทน (Agent)</span>;
    }
  };
  ...
  return (
    <div id="organization_structure_view" className="space-y-6 max-w-7xl mx-auto pb-12">
      {/* Top Banner */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-sky-50 via-sky-100 to-slate-50 border border-sky-50/40 p-6 sm:p-8 shadow-[0_4px_12px_rgba(148,163,184,0.10)]">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-slate-900 font-semibold text-xs sm:text-sm flex items-center gap-2 shadow-[0_1px_3px_rgba(148,163,184,0.08)] shadow-indigo-600/30 transition-all"
            >
        ...
        {/* 4 Rollup KPI Cards */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 pt-6 border-t border-sky-50/40/80">
          <div className="bg-[#fcfdff]/60 p-3.5 rounded-xl border border-sky-50/40/60">
      ...
      {/* Filter & Search Bar */}
      <div className="bg-[#fcfdff] border border-sky-50/40 rounded-xl p-4 flex flex-col md:flex-row gap-3 items-center justify-between shadow-[0_1px_3px_rgba(148,163,184,0.08)]">
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full bg-[#fcfdff] border border-sky-50/40 rounded-lg pl-9 pr-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-indigo-500"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          <select
            ...
            className="bg-[#fcfdff] border border-sky-50/40 rounded-lg px-3 py-2 text-xs text-slate-800 focus:outline-none focus:border-indigo-500"
          >
          ...
          <button
            ...
            className="px-3 py-2 rounded-lg bg-[#f0f9ff] hover:bg-slate-200 text-xs text-slate-800 transition-all"
          >
      ...
      <div className="space-y-4">
        {regionalManagers.map((rm) => {
          ...
          return (
            <div key={rm.id} className="bg-[#fcfdff] border border-sky-50/40 rounded-2xl overflow-hidden shadow-[0_1px_3px_rgba(148,163,184,0.08)] transition-all">
              {/* Region Header */}
              <div 
                onClick={() => toggleExpand(rm.id)}
                className="p-5 bg-gradient-to-r from-amber-950/30 via-sky-100 to-slate-50 border-b border-sky-50/40 cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-[#f0f9ff]/40"
              >
                <div className="flex items-center gap-3">
                  <button className="p-1 rounded bg-[#f0f9ff] text-slate-800">
                  ...
                  <img
                    ...
                    alt={rm.name}
                    className="w-11 h-11 rounded-full object-cover border-2 border-blue-600 shadow-[0_1px_3px_rgba(148,163,184,0.08)]"
                  />
                  <div>
              ...
              {isRmExpanded && (
                <div className="p-4 sm:p-6 space-y-4 bg-[#fcfdff]/40">
                  {rmCenters.map((cm) => {
                    ...
                    return (
                      <div key={cm.id} className="bg-[#fcfdff]/80 border border-sky-50/40 rounded-xl overflow-hidden">
                        {/* Center Row */}
                        <div 
                          onClick={() => toggleExpand(cm.id)}
                          className="p-4 bg-[#fcfdff] border-b border-sky-50/40 cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-3 hover:bg-[#f0f9ff]/40"
                        >
                          <div className="flex items-center gap-3">
                            <button className="p-1 rounded bg-[#f0f9ff] text-slate-800">
                            ...
                            <img
                              ...
                              alt={cm.name}
                              className="w-9 h-9 rounded-full object-cover border-2 border-indigo-500 shadow-[0_1px_3px_rgba(148,163,184,0.08)]"
                            />
                            <div>
                              <div className="flex items-center gap-2">
                        ...
                        {/* Units Under Center */}
                        {isCmExpanded && (
                          <div className="p-3 sm:p-4 space-y-3 bg-[#fcfdff]/60">
                            {cmUnits.map((um) => {
                              ...
                              return (
                                <div key={um.id} className="bg-[#fcfdff] border border-sky-50/40 rounded-lg p-3">
                                  <div 
                                    onClick={() => toggleExpand(um.id)}
                                    className="flex items-center justify-between cursor-pointer"
                                  >
                                    <div className="flex items-center gap-2.5">
                                      <button className="p-0.5 rounded bg-[#f0f9ff] text-slate-800">
                                    ...
                                    <div className="flex items-center gap-3 text-right">
                                      ...
                                      <button
                                        ...
                                        className="px-2 py-0.5 rounded bg-[#f0f9ff] hover:bg-slate-200 text-[11px] text-slate-800"
                                      >
                                        ดู
                                      </button>
                                    </div>
                                  ...
                                  {isUmExpanded && (
                                    <div className="mt-3 pt-3 border-t border-sky-50/40/80 pl-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">
                                      {umAgents.map((ag) => (
                                        <div 
                                          key={ag.id}
                                          onClick={() => setSelectedMemberId(ag.id)}
                                          className="p-2 rounded-lg bg-[#fcfdff]/80 border border-sky-50/40/80 flex items-center justify-between cursor-pointer hover:border-sky-50/40 hover:bg-[#fcfdff] transition-all"
                                        >
                                          <div className="flex items-center gap-2">
                                            <img
